...
const input = injectIntl( ( props ) => {
	...
	const floatlInputClass = cn('floatl', {'floatl--active': input.value }, props.className)
	...
	return (
		<FormGroup className={floatlInputClass} controlId={input.name} validationState={validationState}>
			...
			<FormControl {...input} componentClass="textarea" rows={rows} className="floatl__input" placeholder={placeholder} />
