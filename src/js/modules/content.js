...
// import { createAxiosInstance } from '../configs/core'
...
// 	receivedAt: Date.now()
// })
// export const appendContent = (data) => ({
// 	type: APPEND_CONTENT,
// 	data,
// 	key: _.keys(data).pop(),
// 	receivedAt: Date.now()
...
// 			let stateEntries = _.get(state, `${action.key}.entries`, []).slice()
// 			let entries = stateEntries.map((stateEntry, index) => {
...
// 			entries = _.get(state, `${action.key}.entries`, []).slice()
// 			const entry = action.data || {}
// 			if (entries[action.index]) entries[action.index] = entry
// 			return _.merge({}, state, {[`${action.key}`]: {entries}})
// 		case REFRESH_ONE:
// 			_.set(loaders, action.key, false)
// 			stateEntries = _.get(state, `${action.key}.entries`, [])
// 			const actionEntry = _.get(action.data, `${action.key}.entries[0]`, {})
// 			entries = _.reduce(stateEntries, (res, entry, key) => {
...
// 			_.set(loaders, action.key, false)
// 			stateEntries = _.get(state, `${action.key}.entries`, [])
// 			const actionEntries = _.get(action.data, `${action.key}.entries`, [])
// 			entries = _.unionBy(actionEntries, stateEntries, 'id')
...
// 			_.set(loaders, action.key, false)
// 			stateEntries = _.get(state, `${action.key}.entries`, [])
// 			actionEntries = _.get(action.data, `${action.key}.entries`, [])
// 			entries = _.unionBy(stateEntries, actionEntries, 'id')
...
// 			return Object.assign({}, state, {loaders})
// 		case CLEAR_CONTENT:
// 			const nextState = _.omit(Object.assign({}, state), action.params)
